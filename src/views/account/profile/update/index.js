...
class UpdateInformation extends Component {
  ...
  updateAccount = () => {
    ...
    const { authUser } = this.props;
    Keyboard.dismiss();
    if (userName.trim() === "" || !alphanumeric(userName) || userName.length > 50) {
      return Alert.alert(
        "Thông báo",
      ...
      (!alphanumeric(nameAccount) || nameAccount.length > 100) &&
      account.trim() !== ""
    ) {
      Alert.alert(
        "Thông báo",
  ...
  GetCTV = () => {
    ...
    GetCTVDetail({
      ...
      console.log("abccccc", res.data.WARD)
      this.setState({
